...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="likebdz/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/likeBDZ/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Docker Image Push') {
            ...
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_likebdz') {
                        oDockImage.push()
                    }
                }
            }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'likebdz@outlook.kr')
